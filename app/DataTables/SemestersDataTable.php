...
class SemestersDataTable extends DataTable
{
    ...
    public function dataTable($query)
    {
        return datatables()
            ->eloquent($query)
            ->addColumn('Action', 'admin.semesters.action')
            ->rawColumns([
                'Action',
            ])
            ...
            ->filterColumn('academic_year', function ($query, $keyword) {
                ...
                $query->whereRaw("academic_years.title like ?", ["%{$keywords}%"]);
             })
            ->addIndexColumn();
    }
    ...
    public function query(Semester $model)
    {
        ...
        return $model->select('semesters.*', 'academic_years.title as academic_year')
    ...
    protected function getColumns()
    {
        return [
            ...
            Column::make('start_date')->title(trans('general.start_date')),
            Column::make('end_date')->title(trans('general.end_date')),
            Column::make('academic_year')->title(trans('general.academic_year')),
